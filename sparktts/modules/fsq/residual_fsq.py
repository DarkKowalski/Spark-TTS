# From by https://github.com/arghyasur1991/Spark-TTS
...
class ResidualFSQ(Module):
    ...
    def codebooks(self):
        ...
        # Don't use torch.stack
        codebooks = torch.cat([cb.unsqueeze(0) for cb in codebooks], dim=0)
        return codebooks

    def get_codes_from_indices(self, indices, onnx_export_mode=True):
    ...
    def get_output_from_indices(self, indices, onnx_export_mode=True): # Thread the flag
    ...
    def detokenize(self, indices, onnx_export_mode=True): # Added detokenize
    ...
    def forward(self, x, return_all_codes=False, rand_quantize_dropout_fixed_seed=None, onnx_export_mode=True): # Thread the flag
...
class GroupedResidualFSQ(Module):
    ...
    def get_codes_from_indices(self, indices, onnx_export_mode=True): # Thread flag
    ...
    def get_output_from_indices(self, indices, onnx_export_mode=True): # Thread flag
        # indices for GroupedResidualFSQ is expected to be a list/tuple of index tensors, one per group
        outputs = tuple(
    ...
    def detokenize(self, indices, onnx_export_mode=True): # Added detokenize for GroupedResidualFSQ
        # This is an alias for get_output_from_indices
        # `indices` for GroupedResidualFSQ's get_output_from_indices is a list/tuple of index tensors
        return self.get_output_from_indices(indices, onnx_export_mode=onnx_export_mode)

    def forward(self, x, return_all_codes=False, onnx_export_mode=True): # Thread flag
